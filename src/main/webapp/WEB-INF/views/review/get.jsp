...
<body>
	...
	<input type="hidden" id="number" value="${review.number }" readonly/>
	...
	<!-- 게시글로 돌아가기 버튼 -->
	<div class="topbtnBox">
		<c:url value="/review/list" var="listLink"></c:url>
		<a href="${listLink }">
		...
		<c:if test="${review.member_userId == userIdValue}" >
		<!-- 삭제버튼 -->
			<c:url value="/review/remove" var="removeLink"></c:url>
			...
			<!-- 수정버튼 -->
			<c:url value="/review/modify" var="modifyLink">
	...
	<div id="post_wrap">
		<div class="post_top">
			...
			<p class="top_title">${review.title }</p><br>
			<div class="rowLine"></div><br>
			<p class="txt">카테고리</p><span class="colLine"></span><p>${review.category }</p>
			<p class="txt space1">작성자</p><span class="colLine"></span><p>${review.member_userId }</p>
			<p class="txt space2">일자</p><span class="colLine"></span><p>${review.insertDate }</p>
			<div class="rowLine"></div><br>
		</div>
